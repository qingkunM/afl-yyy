...
static void mark_as_redundant(struct queue_entry* q, u8 state) { //将冗余的测试用例保存
	...
	if (state) {
		...
		tmpy = alloc_printf(
				"\"%s\" is not favorated, and its bitmapsize is %6u\n.",
				tmpy + 1, q->bitmap_size);
		;
		ylen = snprintf(NULL, 0, tmpy);
		ck_write(fdy, tmpy, ylen, NULL);
		ck_free(tmpy);
		close(fdy);
		...
		tmpy =
				alloc_printf(
						"the testcase \"%s\" is favorated again, and its bitmapsize is %6u\n.",
						tmpy + 1, q->bitmap_size);
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
	...
	if (fault == crash_mode) {
...
#ifdef XIAOSA
		//save some trace_plot information
		...
		tmpy = alloc_printf("%s/trace_plot", out_dir);

		fd_xs = open(tmpy, O_WRONLY | O_CREAT | O_APPEND, 0600); //需要追加的模式
		if (fd_xs < 0)
			PFATAL("Unable to create '%s'", fn);
		ck_free(tmpy);
...
static u8 fuzz_one(char** argv) {
	...
	if (pending_favored) { //pending_favored表示待测试的测试用例数量,是约简后的数量,第二轮之后不判断,除非有新的测试用例
	...
	if (not_on_tty)
		ACTF("Fuzzing test case #%u (%u total)...", current_entry,
				queued_paths);
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l) - 1) - EFF_APOS(_p) + 1) //这个宏在字典中才用到
	...
	if (eff_cnt != EFF_ALEN(len) && eff_cnt * 100 / EFF_ALEN(len) > EFF_MAX_PERC) { //计算影响的范围,以及影响范围的比例
...
static void setup_dirs_fds(void) {
	...
	tmp = alloc_printf("%s/trace_mini", out_dir);
	if (mkdir(tmp, 0700))
		PFATAL("Unable to create '%s'", tmp);
	ck_free(tmp);
#endif
...
int main(int argc, char** argv) {
	...
	while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:L")) > 0) { //getopt 系统调用
		...
		case 'D':
			...
			if (N_timeout_given)
				FATAL("Multiple -D options not supported");
			if (sscanf(optarg, "%u", &N_exec_tmout) < 1 || optarg[0] == '-')
	...
	if (fdy < 0) PFATAL("Unable to create '%s'", "out_dir/trace_plot");
	ck_free(tmpy);

	tmpy = alloc_printf("digraph graphname{\n");//提前写入头信息
	ck_write(fdy, tmpy, strlen(tmpy), NULL);
	ck_free(tmpy);
	//--------------------
